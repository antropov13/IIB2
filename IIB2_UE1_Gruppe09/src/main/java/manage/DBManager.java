...
public class DBManager {
	...
	private static java.sql.Connection getDBConnection(String dbName) throws ClassNotFoundException, SQLException {
		...
		String p = "?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC"; // das
																															// ist
																															// notwendig
																															// wegen
																															// Fehler
	...
	/*
	 * public static List<Fachrolle> getUser(String tabelle) throws
	 * ClassNotFoundException, SQLException {
	 * 
	 * String sql = "SELECT * FROM " +tabelle +" ;"; ResultSet r = query(sql);
	 * 
	 * List<Fachrolle> frList = new ArrayList<Fachrolle>(); if
	 * (tabelle.equals("Dezernatmitarbeiter")) { while (r.next()) { Fachrolle fr =
	 * new Fachrolle(); fr.setId(r.getInt(1)); fr.setNachname(r.getString(2));
	 * fr.setVorname(r.getString(3)); fr.setUsername(r.getString(4));
	 * 
	 * fr.setFachrolle(tabelle); frList.add(fr); } } else { while (r.next()) {
	 * Fachrolle fr = new Fachrolle(); fr.setId(r.getInt(1));
	 * fr.setFirmaname(r.getString(2)); fr.setUsername(r.getString(3));
	 * 
	 * fr.setFachrolle(tabelle); frList.add(fr); } } return frList; }
	 */
	...
	public List<Leistungsspektrum> getLeistungen(String sql, Dienstleister dlr)
	...
	public Gebaeude getGebaeude(String sql) throws ClassNotFoundException, SQLException {
		...
		while (rs.next()) {
			...
			gebaeude.setGuid(rs.getString(7));
		}
		return gebaeude;
	}
	...
	public List<Stockwerk> getStockwerk(String sql) throws ClassNotFoundException, SQLException {
		Connection con = getDBConnection(datenbankname);
		Statement stmt = con.createStatement();
		ResultSet rs = stmt.executeQuery(sql);
	...
	public List<Raum> getRaum(String sql) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = con.createStatement();
		ResultSet rs = stmt.executeQuery(sql);
		List<Raum> rL = new ArrayList<Raum>();
		Raum raum = null;
		boolean val = rs.next();
		if (val == false) {
			return null;
	...
	public List<LnRaumWand> getLnRaumWand(String sql) throws ClassNotFoundException, SQLException {
		Connection con = getDBConnection(datenbankname);
		Statement stmt = con.createStatement();
		ResultSet rs = stmt.executeQuery(sql);
		List<LnRaumWand> lrwL = new ArrayList<LnRaumWand>();
		LnRaumWand lrw = null;
		boolean val = rs.next();
		if (val == false) {
			return null;
			...
			while (val) {
				lrw = new LnRaumWand();
				lrw.setId(rs.getInt(1));
				lrw.setRaum_id(rs.getInt(2));
				lrw.setWand_id(rs.getInt(3));
				lrwL.add(lrw);
				val = rs.next();
			}
		}
	...
	/*
	 * public List<LnAuftragDln> getlnauftragdln(String sql) throws
	 * ClassNotFoundException, SQLException { List<LnAuftragDln> lnauftragdln =
	 * null; Connection con = getDBConnection(datenbankname); Statement stmt =
	 * con.createStatement(); ResultSet r = stmt.executeQuery(sql); LnAuftragDln dln
	 * = null ; boolean val = r.next(); if(val==false){ return lnauftragdln; } else
	 * { lnauftragdln = new ArrayList<LnAuftragDln>(); while (val) { dln = new
	 * LnAuftragDln(); dln.setId(r.getInt(1));
	 * dln.setDienstleistung_id(r.getInt(2));
	 * dln.setAuftrag_id(auftrag_id);(r.getString(3));
	 * dln.setHaeufigkeit(r.getString(4)); dln.setDmaId(r.getInt(5));
	 * lnauftragdln.add(dln); val=r.next(); } con.close(); // Very important! }
	 * return lnauftragdln; }
	 */
